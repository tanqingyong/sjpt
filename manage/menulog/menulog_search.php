...
$sql = "SELECT ml.username as username,ml.ip as ip,ml.action_time as action_time,m.parent_id as parent_id,m.id as menu_id
		FROM menu_log as ml 
	 	INNER JOIN menu as m ON ml.menu_id = m.id WHERE 1=1 ";
...
if(!empty($user_name)){
	...
	$sql .= " AND ml.username='$user_name' ";
	$sql_count .= " AND ml.username='$user_name' ";
}
