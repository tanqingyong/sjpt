...
<div class="right-data">
	<div class="content-info">
		<form id="signup-user-form" method="post"
			action="/manage/user/update_password.php"
			onsubmit="return check_value();" class="validator" >
		
		<div class="field username"><label for="signup-username">用户名</label> <input
			type="text" size="30" name="username" id="signup-username"
			class="f-input" disabled="disabled"
			value="<?php
			echo $arr_user ['username'];
			?>" /></div>
		<div class="field password"><label for="signup-password">密码</label> <input
			type="password" size="30" name="password1" id="signup-password"
			class="f-input" onkeyup="check_password()" /><span id="pwd_msg"
			style="color: red"></span> <span class="hint">为了您的帐号安全，密码最少设置为8个字符，且至少包含一个数字和一个字母</span>
		</div>
		...
		<div class="field password">
			<label>&nbsp;&nbsp;</label>
			<a class="submit-btn" href="javascript:void(0);" onclick="cha_xun();">修改</a>
		</div>
		</form>
	</div>
</div>
