...
function cha_xun(){
	$("form:first").attr('action','<?php echo WEB_ROOT;?>/manage/operation/flow/login_page_data.php');
    $("form:first").submit();
}
function export_excel(){
	$("form:first").attr('action','<?php echo WEB_ROOT;?>/manage/operation/flow/export_login_page_data.php');
    $("form:first").submit();
}
                        ...
                        <select id="type" name="type" class="se_ect1">
                            <option></option>
                             <?php  echo ''.Utility::Option(get_paytype_option('product_login_page','pindao'),$pay_type).''; ?>
                       </select>
                    </td>
								   <td>
									<label>查询从</label>
                         ...
                         <div class="table-data-box">
							   ...
							   <?php foreach($datas as $data){?>
										<tr><td><?php echo $data['date'];?></td>
											...
											<td><?php echo $data['login_rate']*100;echo "%";?></td>
										</tr>
										<?php }?>
										<?php if(!$datas){?>
