...
function cha_xun(){
	$("form:first").attr('action','<?php echo WEB_ROOT;?>/manage/operation/user_center/user_values_data.php');
    $("form:first").submit();
}
function export_excel(){
	$("form:first").attr('action','<?php echo WEB_ROOT;?>/manage/operation/user_center/export_user_values_data.php');
    $("form:first").submit();
}
                        ...
                        <select id="start1" name="start1" class="se_ect1">
                            ...
                            <option></option>
                             <?php global $option_month; echo ''.Utility::Option($option_month,$end1).''; ?>
                       </select>
									月</td>
									<td>
                         ...
                         <div class="table-data-box">
                                   <table cellpadding="0" cellspacing="0"   style="width:100%" >
								    <thead>
								    	<tr><td>年</td><td>月份</td><td>用户ID</td><td>最近购买时间</td><td>购买频率</td>
								    	<td>购买商品种类</td><td>平均每次消费额</td><td>单次最高消费额</td></tr>
								    </thead>		 
									<tbody>		
							   <?php foreach($datas as $data){?>
										<tr><td><?php echo $data['year'];?></td>
											<td><?php echo $data['month_num'];?></td>
