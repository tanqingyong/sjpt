...
function cha_xun(){
	$("form:first").attr('action','<?php echo WEB_ROOT;?>/manage/operation/whole_site/out_visit_url.php');
    $("form:first").submit();
}
function export_excel(){
	$("form:first").attr('action','<?php echo WEB_ROOT;?>/manage/operation/whole_site/export_out_visit_url.php');
    $("form:first").submit();
}
						      ...
						      <tr><td>日期</td><td>URL</td><td>PV</td><td>UV</td><td>IP</td><td>跳出率</td></tr> 
								...
								<tbody>
								   <?php foreach($datas as $data){?>			 
											<tr><td style="width:90px; word-break:break-all"><?php echo $data['data_date'];?></td>
												<td style="width:300px; word-break:break-all"><?php echo $data['url'];?></td>
