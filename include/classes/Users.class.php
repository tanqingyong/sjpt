...
class Users
{
	...
	static public function GetAuthority($user_id) {
		$user_auth = DB::GetTableRows('permission', array(
					'user_id' => $user_id,
		));
		if ($user_auth)  return $user_auth;
		return array();
	}
	
	static public function GetRole($user_id) {
		$user_role = DB::GetTableRows('role', array(
					'user_id' => $user_id,
		));
