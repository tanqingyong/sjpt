...
function template($tFile,$arr_var) {
	...
	include_once (DIR_VIEWS ."/manage_navigation.php");//add header content
 ...
 * 
 */
function get_full_menus(){
	if(!isset($_SESSION['menu'])){
		$_SESSION['menu'] = DB::GetTableRows('menu');
	}
 ...
 */
function get_media_by_user($user_id,$resource_id,$dept_id,$grade){
	$mediaList = array();
	$sql = "select function_name from fuction_city join permission 
		where user_id = $user_id and data_resource_id = $resource_id and department_id = $dept_id and grade = $grade
			and permission.role_id = 3 and menu_id =22 and function_id = fuction_city.id and fuction_city.id !=0 ";
	$result = DB::GetQueryResult($sql,false);
	
	foreach ($result as $_row){
		$mediaList[] = $_row['function_name'];
	}
	return $mediaList;
}
/*
...
function is_manager_sys() {
	$user_auth = Session::Get('user_auth');
	if($user_auth){
		foreach ($user_auth as $permission){
			if($permission['grade'] ==4 && $permission['data_resource_id'] ==='0' && $permission['department_id'] ==='0'){
				return true;
			}
		}
	}
...
function is_manager_resuorce($data_resource_id){
	$user_auth = Session::Get('user_auth');
	if(is_manager_sys()){
		return true;
	}
	if($user_auth){
		foreach ($user_auth as $permission){
			if($permission['grade'] ==4 && $permission['data_resource_id'] ===$data_resource_id && $permission['department_id'] ==='0'){
				return true;
			}
		}
	}
...
function is_manager_dept($department_id,$super=false, $weak=false) {
	if(is_manager_sys()){
		return true;
	}
	$resource_id = get_data_resource_dept($department_id);
	if(is_manager_resuorce($resource_id)){
		return true;
	}
	$user_auth = Session::Get('user_auth');
	if($user_auth){
		foreach ($user_auth as $permission){
			if($permission['grade'] ==4 && $permission['department_id'] == $department_id){
				return true;
			}
		}
	}
...
function is_user_exist($user_id){
	$arr_eu = Table::Fetch ( 'users', $user_id, 'id' );
	if($arr_eu){
		return true;
	}else{
		return false;
...
function correct_hsxm($hsxm,$from_city_name,$arr_k3_d){
	...
	function upload_file($input_name, $upload_dir){
		if(($_FILES[$input_name]["type"] == 'application/vnd.ms-excel'||$_FILES[$input_name]["type"] == 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet')&&($_FILES[$input_name]["size"] < 20000000)){
			...
			if ($_FILES[$input_name]["error"] > 0){
			    ...
			    if(move_uploaded_file($_FILES[$input_name]["tmp_name"], $upload_dir . $_FILES[$input_name]["name"])){
	 ...
	 * 获取媒体数组
	 */
    function get_platforms_option(){
    	global $platforms;
        $platforms = array();
        $sql_platform = "select distinct platform_id from platform_update_new where platform_id != '合计' and platform_id !=''";
        
        $platform_datas = DB::Query($sql_platform);
        while ($row = mysql_fetch_array($platform_datas, MYSQL_ASSOC)) {
            $platforms[$row['platform_id']] = $row['platform_id'];
        }
        return $platforms;
    }
    //用于控制用户查询媒体权限使用的查询媒体条件功能
    function get_platforms_for_search(){
    	$platforms_search = array();
	    $user_auth = Session::Get('user_auth');
		if($user_auth){
			foreach ($user_auth as $permission){
				if($permission['grade'] ==1 && $permission['data_resource_id'] ==='1' && $permission['department_id'] ==='3'){
					
					$mediaList =  get_media_by_user($permission['user_id'],1,3,1);
					$result = array();
    ...
    function get_paytype_option($aaa,$bbb){
    	global $paytype;
        $paytype = array();
        $sql = "SELECT  DISTINCT $bbb as 'type' from $aaa ";
        
        $paytype_datas = DB::Query($sql);
        while ($row = mysql_fetch_array($paytype_datas, MYSQL_ASSOC)) {
            $paytype[$row['type']] = $row['type'];
        }
        return $paytype;
    }
